...
function sub() {
    INQKEY=$(echo $1 | cut -f 1 -d '|')
    VALIDATE=$(echo $1 | cut -f 2 -d '|')
	...
	echo "INSERT INTO grid_${ZLEVEL} (qkey, geom, validate) VALUES ('$INQKEY', $GEOM, $VALIDATE );" > $OUTSQL 
...
for ZLEVEL in 17 18; do
    ...
    parallel --nice 10 --progress sub {} ::: $(psql -h guam suvannaket -qAtc "select qkey,validate from grid where validate = 2 OR validate = 1")
